...
# 반복 재생할 오디오 파일을 직접 지정합니다.
# 다른 음원을 사용하려면 아래 경로만 변경하면 됩니다.
AUDIO_FILE = "/home/sos/Capstone_projects/warning.mp3"

# aplay는 MP3를 직접 재생하지 못하므로 프로그램 시작 시 ffmpeg로
# 임시 WAV 파일을 만든 뒤, 확인된 USB ALSA 장치로 재생합니다.
AUDIO_DECODED_WAV_PATH = "/tmp/capstone_warning_playback.wav"

# 실제 재생 테스트에서 정상 확인된 AB13X USB Audio 출력 장치
#
# 터미널 WAV 출력 확인 명령:
# aplay -D plughw:CARD=Audio,DEV=0 -vv /tmp/capstone_warning_playback.wav
AUDIO_OUTPUT_DEVICE = "plughw:CARD=Audio,DEV=0"

AUDIO_EXTENSIONS = ("*.wav", "*.mp3", "*.ogg", "*.m4a", "*.aac")

# 컵과 사람의 분리가 확인되고 해당 사람이 실제로 검출되는 동안
# warning.mp3를 다시 시작하는 간격입니다.
AUDIO_REPEAT_INTERVAL = 3.0

# 추적 대상이 이 시간 동안 실제로 탐지되지 않으면
# 팬/틸트를 초기 위치로 복귀하고 추적 상태를 초기화합니다.
NO_ACTIVITY_RESET_TIME = 4.0

# 프로그램 시작 시에는 오디오 파일 존재 여부와 재생 명령만 확인합니다.
# 실제 시험 재생은 하지 않으므로 프로그램을 켤 때 경고음이 나오지 않습니다.
AUDIO_STARTUP_TEST = False
...
# 이 범위 안으로 들어오면 해당 축을 고정합니다.
# 기존 12px보다 넓혀 YOLO 박스의 작은 흔들림에 모터가 반응하지 않게 합니다.
DEAD_ZONE_X = 32
DEAD_ZONE_Y = 26

# 한 번 고정된 축은 대상이 이 범위 밖으로 확실히 벗어날 때만 다시 움직입니다.
# 잠금 범위와 해제 범위를 다르게 둔 히스테리시스 방식입니다.
...
# 중앙에 들어오면 즉시 정지하고, 미분값만 줄여 재출발 시 튐을 막습니다.
...
# 작은 위치 오차에 과도하게 반응하지 않도록 비례 이득을 낮췄습니다.
PD_KP = 0.34
PD_KD = 0.003

# 검출 위치 변화의 미분값도 충분히 평활화합니다.
...
# 빠른 좌우 왕복을 줄이되 사람이 크게 이동하면 충분히 따라가도록 설정합니다.
MAX_SERVO_SPEED = 85.0

# 검출 좌표 저역 통과 필터입니다. 기존 0.08은 반응 지연이 너무 커서
# 최신 검출 위치를 45% 반영하도록 높였습니다. 중앙 떨림은 데드존이 막습니다.
TARGET_FILTER_ALPHA = 0.45

# 추적 제어 갱신 간격입니다. 별도의 S-curve 출력 지연 없이 바로 출력합니다.
...
# S-curve/jerk 보간 스레드는 제거했습니다. 메인 추적 제어가 계산한 각도를
# 즉시 PCA9685에 전달하되, 서보가 구분하기 어려운 극미세 변화만 생략합니다.
...
def prepare_audio_for_alsa(audio_path):
    """
    warning.mp3를 aplay가 확실하게 재생할 수 있는 PCM WAV로 변환합니다.

    MP3를 ffplay로 직접 재생하면 USB ALSA 장치가 아닌 기본 출력으로
    나갈 수 있으므로, ffmpeg로 한 번만 변환한 뒤 기존에 확인된
    AUDIO_OUTPUT_DEVICE를 aplay에 직접 지정합니다.
    """
    ...
    if shutil.which("ffmpeg") is None:
        ...
        print(f"[AUDIO CHECK][FAIL] {audio_last_error}")
        return None
    ...
    try:
        ...
        print(
            f"[AUDIO CHECK][OK] MP3를 ALSA 재생용 WAV로 변환했습니다: "
            f"{AUDIO_DECODED_WAV_PATH}"
        )
        ...
        print(
            f"[AUDIO CHECK][FAIL] MP3 변환 중 오류: "
            f"{audio_last_error}"
        )
        return None
...
def start_audio_playback(audio_path):
    ...
    if command is None:
        audio_last_error = (
            "ALSA 재생 명령을 만들지 못했습니다. "
            "aplay 설치 여부와 MP3 변환 결과를 확인하십시오."
        )
...
def check_audio_configuration(audio_path):
    ...
    if audio_path is None:
        audio_last_error = "warning.mp3 또는 변환된 WAV 파일을 찾지 못했습니다."
    ...
    try:
        ...
        try:
            ...
            print(
                "[AUDIO CHECK] 방금 warning.mp3 변환본 앞부분을 "
...
# 마지막으로 실제 출력한 각도를 기억해 너무 작은 반복 명령만 생략합니다.
...
AUDIO_PATH = prepare_audio_for_alsa(
    AUDIO_SOURCE_PATH
)

if AUDIO_SOURCE_PATH is not None:
    print(
        f"[INFO] 선택한 오디오 파일: {AUDIO_SOURCE_PATH}"
    )

    if AUDIO_PATH is not None:
        print(
            f"[INFO] ALSA 재생 파일: {AUDIO_PATH}"
        )
    ...
    print(
        "[WARNING] /home/sos/Capstone_projects/warning.mp3를 "
        "찾지 못했습니다."
    )
...
# 중앙에 들어온 축은 고정하고, 더 넓은 해제 범위를 벗어날 때만
# 다시 움직이는 히스테리시스 상태입니다.
...
try:
    while True:
        ...
        # 다음 조건을 모두 만족할 때만 경고음을 반복합니다.
        # 1) 컵과 사람의 거리 분리가 확인되어 얼굴 추적 상태일 것
        # 2) 현재 프레임에서 등록된 사람이 실제로 검출될 것
        # 3) 현재 사람-컵 거리도 분리 기준 이상일 것
        #
        # PERSON_HOLD_TIME 동안 마지막 얼굴 좌표를 유지하는 target_held 상태는
        # 실제 검출이 아니므로 오디오를 즉시 중지합니다.
        audio_should_play = (
            real_target_detected
            and current_distance_ratio is not None
            and current_distance_ratio
            >= PERSON_SEPARATION_DISTANCE_RATIO
            and not returning_to_initial
        )
        ...
        # 중앙 고정이 풀리는 범위를 어두운 청록색 선으로 표시합니다.
        cv2.rectangle(
            frame,
            (
        ...
        if face_target is not None:
            ...
            if (
                ...
                # 중앙 고정 히스테리시스
                # -------------------------------------------
                # 한 번 중앙에 들어온 축은 작은 검출 흔들림만으로 풀리지 않습니다.
                # 대상이 release zone 밖으로 확실히 이동했을 때만 다시 추적합니다.
                if pan_axis_locked:
                ...
                # 추적 중에는 출발 가속 램프를 거치지 않고 PD가 계산한
                # 목표 속도를 즉시 사용합니다. 중앙 잠금 시에도 바로 정지합니다.
                pan_servo_velocity = (
                    0.0
                    if pan_axis_locked
                    else target_pan_velocity
                )

                tilt_servo_velocity = (
                    0.0
                    if tilt_axis_locked
                    else target_tilt_velocity
                )
        ...
        # ====================================================
            ...
            # 추적 대상이 없으면 남아 있던 추적 속도를 즉시 제거합니다.
            # 초기 위치 복귀 속도는 아래 복귀 제어부에서 별도로 계산합니다.
            if not returning_to_initial:
                pan_servo_velocity = 0.0
                tilt_servo_velocity = 0.0
        ...
        if tracking_state == "WAITING_PAIR":
            ...
            if angle_limit_active:
                ...
                and pan_axis_locked
                and tilt_axis_locked
            ):
                status_text = (
                    "PERSON FACE CENTERED - HOLDING"
                )
        ...
        cv2.putText(
            frame,
            "Servo output: DIRECT (no S-curve)",
            (
                frame_width - 390,
                frame_height - 15
            ),
