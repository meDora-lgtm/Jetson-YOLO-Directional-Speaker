...
# 재생할 오디오 파일입니다. 사람 한 명을 처음 등록할 때 딱 한 번 재생합니다.
...
# 오디오 실행 자체가 실패한 경우 이 시간 뒤 초기 위치로 복귀합니다.
...
# 복귀 완료 후 같은 사람이 계속 화면에 남아 있을 때 오디오가 반복되는 것을 막습니다.
# 사람이 화면에서 이 시간 이상 사라져야 다음 사람 추적을 다시 허용합니다.
...
def select_initial_person(persons, frame_width, frame_height):
    """
    처음 등록할 사람을 고릅니다.

    화면에서 크게 보이는 사람을 우선하고, 비슷한 크기라면 화면 중심에
    가까운 사람을 선택합니다.
    """
    ...
    screen_center_y = frame_height / 2.0
    frame_diagonal = max(1.0, math.hypot(frame_width, frame_height))

    def score(person):
        center_x, center_y = box_center(person)
        center_penalty = math.hypot(
            center_x - screen_center_x,
            center_y - screen_center_y,
        ) / frame_diagonal

        return detection_area(person) * (1.0 - 0.25 * center_penalty)
...
print("[INFO] 한 사람을 고정 추적하고 오디오를 한 번 재생합니다.")
print("[INFO] 오디오 종료 후 팬과 틸트를 초기 위치로 복귀합니다.")
...
try:
    ...
    # 상태:
    # WAITING   : 새 사람을 기다림
    # TRACKING  : 등록한 한 사람을 추적하며 오디오 1회 재생
    # RETURNING : 오디오 종료 후 초기 위치 복귀
    ...
    audio_started_time = 0.0

    # 시작할 때는 바로 한 사람을 받을 수 있습니다.
    ...
    while True:
        ...
        # ----------------------------------------------------
        # 대기 상태: 처음 보이는 한 사람을 등록하고 오디오 1회 시작
        # ----------------------------------------------------
            ...
            if not detection_armed:
                if persons:
                    ...
                    if clear_since is None:
                        ...
                        detection_armed = True
                        clear_since = None
                        print("[INFO] 다음 사람 탐지를 다시 허용합니다.")

            if detection_armed and persons:
                tracked_person = select_initial_person(
                    persons,
                    frame_width,
                    frame_height,
                )
                ...
                audio_started_time = current_time

                tracking_state = "TRACKING"
                detection_armed = False
                clear_since = None

                print("[INFO] 한 사람을 등록하고 추적을 시작합니다.")
        ...
        if tracking_state == "TRACKING":
            ...
            # 오디오는 반복하지 않습니다.
            # 한 번 시작한 aplay가 끝나면 바로 초기 위치 복귀로 전환합니다.
            audio_finished = (
                audio_process is not None
                and audio_process.poll() is not None
            )

            audio_failed_or_unavailable = (
                audio_process is None
                and current_time - audio_started_time
                >= AUDIO_FAILURE_RETURN_DELAY
            )

            if audio_finished or audio_failed_or_unavailable:
                audio_process = stop_audio_playback(audio_process)
                ...
                print("[INFO] 오디오 1회 재생이 끝나 초기 위치로 복귀합니다.")
        ...
        if tracking_state == "RETURNING":
            ...
            if pan_return_complete and tilt_return_complete:
                ...
                print(
                    "[INFO] 초기 위치 복귀 완료. "
                    "현재 사람이 화면에서 나가면 다음 탐지를 준비합니다."
                )

        # ----------------------------------------------------
        # 화면 상태 표시
        # ----------------------------------------------------
        if tracking_state == "WAITING" and detection_armed:
            status_text = "WAITING FOR ONE PERSON"
        elif tracking_state == "WAITING":
            status_text = "DONE - WAITING PERSON TO LEAVE"
        elif tracking_state == "RETURNING":
            status_text = "AUDIO DONE - RETURNING"
        elif target_held:
            ...
            status_text = "TRACKING ONE PERSON + AUDIO ONCE"
        ...
        cv2.putText(
            frame,
            "Detection: PERSON only / Audio: one playback",
            (10, frame_height - 15),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.58,
            (200, 255, 200),
        ...
        if ENABLE_LOCAL_PREVIEW:
            cv2.imshow("Person Tracking - One Audio", frame)
            key = cv2.waitKey(1) & 0xFF
        else:
            key = 255

        # r: 오디오를 중지하고 즉시 초기 위치 복귀
        if key == ord("r"):
            audio_process = stop_audio_playback(audio_process)
            tracking_state = "RETURNING"
            tracked_person = None
