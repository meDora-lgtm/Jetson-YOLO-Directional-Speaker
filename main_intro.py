...
# 재생할 오디오 파일입니다.
# 현재 추적 중인 사람이 화면에 있는 동안 끝날 때마다 계속 반복 재생합니다.
script_dir = os.path.dirname(os.path.abspath(__file__))
AUDIO_FILE = os.path.join(script_dir, "assets", "audio", "hello_sos.mp3")
...
# 오디오 한 번이 끝난 뒤 다음 반복 재생까지의 간격입니다.
AUDIO_REPEAT_GAP = 0.35

# 오디오 실행 자체가 실패한 경우 다시 시도하기까지 사용할 기준 시간입니다.
...
print("[INFO] 현재 추적 중인 사람이 보이는 동안 오디오를 계속 반복 재생합니다.")
print("[INFO] 추적 중인 사람이 사라지면 오디오를 멈추고 초기 위치로 복귀합니다.")
...
try:
    ...
    # TRACKING      : 현재 사람이 보이는 동안 추적하며 오디오를 계속 반복
    # RETURNING     : 현재 사람이 사라진 후 초기 위치 복귀
    ...
    # 현재 사람의 오디오 반복 횟수입니다. 횟수 제한 없이 계속 증가합니다.
    audio_play_count = 0
    ...
    while True:
            ...
            if (
                ...
                if group_candidates:
                    ...
                    print(
                        ...
                        f"오디오 반복 재생 시작 ({audio_play_count}회차)"
                    )
                else:
                    tracking_state = "WAITING_GROUP"
        ...
        # ----------------------------------------------------
        # 등록한 현재 사람을 추적하며 사람이 보이는 동안 오디오를 계속 재생
        # ----------------------------------------------------
        face_target = None
        target_held = False
        ...
        if tracking_state == "TRACKING":
            ...
            # 추적 중인 사람이 완전히 사라졌으면 즉시 오디오를 멈추고
            # 초기 위치 복귀 상태로 전환합니다.
            if face_target is None:
                ...
                print(
                    f"[INFO] {current_person_index + 1}/{group_total}번 사람이 "
                    "사라져 오디오를 멈추고 초기 위치로 복귀합니다."
                )
        ...
        if tracking_state == "RETURNING":
            ...
            if pan_return_complete and tilt_return_complete:
                ...
                if current_person_index < group_total:
                    ...
                    clear_since = None
                    print(
                        f"[INFO] 등록된 사람 {group_total}명의 처리를 완료했습니다."
                    )
        ...
        if tracking_state == "WAITING_GROUP":
            ...
            status_text = "PERSON LOST - RETURNING"
        elif target_held:
            ...
            status_text = "TRACKING PERSON + AUDIO LOOP"
        ...
        if audio_last_error is not None:
            ...
            audio_status = f"PLAYING LOOP {audio_play_count}"
        elif tracking_state == "TRACKING":
            audio_status = f"RESTART GAP {audio_play_count}"
        elif tracking_state == "RETURNING":
        ...
        cv2.putText(
            frame,
            "Detection: PERSON only / Audio: continuous while person is detected",
            (10, frame_height - 15),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.58,
            (200, 255, 200),
        ...
        if ENABLE_LOCAL_PREVIEW:
            cv2.imshow("Sequential Multi-Person Tracking - Continuous Audio", frame)
