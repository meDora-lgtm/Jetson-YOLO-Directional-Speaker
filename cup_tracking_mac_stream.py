...
# 스크립트와 같은 폴더(Capstone 폴더)에서 오디오 파일을 자동 검색합니다.
# 특정 파일을 지정하려면 예: AUDIO_FILE = "warning.wav"
AUDIO_FILE = None

# 지원 확장자 우선순위
AUDIO_EXTENSIONS = ("*.wav", "*.mp3", "*.ogg", "*.m4a", "*.aac")

# 대상이 탐지되는 동안 오디오를 다시 시작하는 간격
AUDIO_REPEAT_INTERVAL = 2.0

# 대상이 몇 초 동안 보이지 않으면 추적 상태를 완전히 초기화할지
NO_ACTIVITY_RESET_TIME = 2.0
...
def start_audio_playback(audio_path):
    ...
    if command is None:
        print(
            "[WARNING] 오디오 재생 프로그램을 찾지 못했습니다. "
            "WAV는 aplay, 그 외 형식은 ffplay 설치를 권장합니다."
        )
        return None
    ...
    try:
        ...
        print(
            f"[WARNING] 오디오 재생 실패: {audio_error}"
        )
        return None
...
gst_output_pipeline = (
    ...
    f"jpegenc quality={STREAM_JPEG_QUALITY} ! "
    "rtpjpegpay pt=26 ! "
    f"udpsink host={MACBOOK_IP} port={STREAM_PORT} "
...
if not stream_writer.isOpened():
    ...
    raise RuntimeError(
        "맥북 영상 스트림을 열 수 없습니다. "
        "OpenCV의 GStreamer 지원 여부와 네트워크 설정을 확인하십시오."
    )

print(
    f"[INFO] 맥북 {MACBOOK_IP}:{STREAM_PORT}로 "
    "실시간 영상을 송출합니다."
)
print(
    "[INFO] 맥북 화면에서는 키 입력을 받을 수 없으므로 "
    "젯슨 터미널에서 Ctrl+C로 종료하십시오."
)
...
# 추적 대상이 마지막으로 실제/유지 탐지된 시각
last_activity_time = current_time

# 2초 소실 후 초기 상태로 되돌릴지 표시
...
try:
    while True:
        ...
        if audio_should_play:
            ...
            # 탐지가 끝나면 남아 있는 오디오도 즉시 정지
            audio_process = stop_audio_playback(
                audio_process
            )

            # 다음 탐지가 시작되면 즉시 한 번 재생되도록 초기화
        ...
        # ====================================================
        # 최종 처리 화면을 맥북으로 실시간 송출
        # ====================================================

        if (
            frame_width != STREAM_WIDTH
            or frame_height != STREAM_HEIGHT
        ):
