...
MODEL_PATH = os.path.join(script_dir, "assets", "models", "yolo26n.engine")
...
script_dir = os.path.dirname(os.path.abspath(__file__))
AUDIO_FILE = os.path.join(script_dir, "assets", "audio", "warning.mp3")
...
try:
    while True:
        ...
        # TRACKING_PERSON 상태에 들어왔다는 사실 자체가 이미
        # 사람과 컵의 분리가 확인되었다는 뜻입니다.
        # 따라서 추적을 시작한 뒤에는 컵이 화면 밖으로 나가거나
        # 일시적으로 검출되지 않아도 얼굴 목표가 유지되는 동안 재생합니다.
        #
        # 얼굴이 PERSON_HOLD_TIME보다 오래 사라져 face_target이 None이 되거나
        # 초기 위치 복귀가 시작되면 오디오를 즉시 중지합니다.
        audio_should_play = (
            tracking_state == "TRACKING_PERSON"
            and face_target is not None
